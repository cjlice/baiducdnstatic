...
;(function (window, document, Math) {
	...
	function JRoll(el, options) {
		...
		if (isMobile) {
			...
			document.addEventListener("touchcancel", me._touch.bind(me, me.scroller), false);
		} else {
	...
	JRoll.prototype = {
		...
		//scrollStart默认事件
		_defaultScrollStart : function (e) {
			var me = this;
			if (me.options.scrollBarFade) {
		...
		_touch : function (el, e) {
			...
			case "mousemove":
				...
				if (me.status === "start") {
					if (t.length === 1 && me.options.scroll) {
						...
						me._execEvent("scrollStart", t);
						me.status = "scroll";
					} else if (me.options.zoom) {
						me._execEvent("zoomStart", t);
						me.status = "zoom";
					}
					return;
				}
			...
			case "mouseup":
				...
				if (me.status === "zoom") {
					...
					me.status = "end";
					me._execEvent("zoomEnd", t);

					return;
				}

				me.scrollTo(me.x, me.y);	//解决滑动过程中第二个手指点击时没复位的bug
